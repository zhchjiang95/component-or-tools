...
export interface DigitRollType {
  setWidth?: (n: number) => void;
  roll?: (n: string | number) => void;
}

/**
 * H5数字滚动效果（原：https://github.com/kele527/digitRoll）
 * @class DigitRoll
 ...
 */
const DigitRoll = (function (this: ThisType, opts: Opts) {
  ...
  this.setWidth();
} as any) as { new (opts: Opts): DigitRollType };
...
DigitRoll.prototype = {
   ...
   * @example
    ...
    */
  roll: function (n: string | number) {
    var self = this;
    this.number = parseInt(n.toString()) + '';
    if (this.number.length < this.width) {
      this.number =
        new Array(this.width - this.number.length + 1).join('0') + this.number;
    } else if (this.number.length > this.width) {
      this.width = this.number.length;
      this.setWidth();
    }
    Array.prototype.forEach.call(
      this.container.querySelectorAll('.num'),
      function (item, i) {
        ...
        item.style.cssText +=
          '-webkit-transition-duration:0s;-webkit-transform:translateY(0)'; //重置位置
        item.innerHTML = gapStr;
        setTimeout(function () {
          item.style.cssText +=
            '-webkit-transition-duration:1s;-webkit-transform:translateY(-' +
            self.rollHeight * gapNum +
            'px)';
        }, 50);
      },
    );
